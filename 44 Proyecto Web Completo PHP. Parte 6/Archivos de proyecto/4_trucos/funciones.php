...
function reservaViaje(){
	...
	$consulta = "SELECT * FROM viajes WHERE nombre='$viaje'";
	$valor = conexionBD($consulta);
	while($row = mysql_fetch_assoc($valor)){
			$capacidad = $row['capacidad'];
	}
	$capacidad = (int)$capacidad;
	$plazas = (int)$plazas;
	$numero_plazas = $capacidad - $plazas;
	
	$consulta2 = "UPDATE viajes SET capacidad=$numero_plazas WHERE nombre='$viaje'";
	conexionBD($consulta2);
	
	//crear una tabla de reservas para que sepa la agencia las plazas que se han vendido
	//Enviar un mail al comprador con el billete
